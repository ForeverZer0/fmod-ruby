...
module FMOD
  class ChannelControl < Handle
    ...
    class MixMatrix
      ...
      def initialize(output_speakers, input_channels)
        if output_speakers < 1
          raise RangeError, "Must have more one or more output speakers."
        end
        if input_channels < 1
          raise RangeError, "Must have more one or more input channels."
        end
        @output_count = output_speakers
        @input_count = input_channels
        @matrix = Array.new(@output_count * @input_count, 0.0)
      end

      def [](speaker, channel)
        @data[speaker + (channel * @output_count)]
      end

      def []=(speaker, channel, value)
        @data[speaker + (channel * @output_count)] = value
      end

      alias_method :rows, :output_count
      alias_method :columns, :input_count
    end


    include Fiddle
    include FMOD::Structs
    ...
    # channels of a stereo sound to arbitrary speakers, see {#set_matrix}.
    #
    ...
    # This is a helper to avoid calling {#set_matrix}.
    #
    ...
    def matrix
      rows = "\0" * SIZEOF_INT
      columns = "\0" * SIZEOF_INT
      FMOD.invoke(:ChannelGroup_GetMixMatrix, self, nil, rows, columns, 0)
      output, input = rows.unpack1('l'), columns.unpack1('l')
      return nil if input.zero? || output.zero?
      data = "\0" * (SIZEOF_FLOAT * (output * input))
      FMOD.invoke(:ChannelGroup_GetMixMatrix, self, data, rows, columns, 0)
      matrix = MixMatrix.new(output, input)
      matrix.instance_variable_set(:@data, data.unpack('f*'))
      matrix
    end

    def matrix=(matrix)
      FMOD.check_type(matrix, MixMatrix)
      data = matrix.instance_variable_get(:@data)
      set_matrix(data,  matrix.rows, matrix.columns)
    end

    ##
    #
    # @param matrix [Array<Float>]
    # @param rows [Integer]
    # @param columns [Integer]
    # @return [self]
    def set_matrix(matrix, rows, columns)
      matrix = matrix.pack('f*')
      FMOD.invoke(:ChannelGroup_SetMixMatrix, self, matrix, rows, columns, 0)
      self
    end
    ...
    def cone_orientation
      vector = FMOD::Structs::Vector.new
