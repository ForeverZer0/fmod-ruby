...
module FMOD
  ...
  @function_signatures = {
    ...
    # System_GetSpeakerPosition: [TYPE_VOIDP],
    ...
    # System_Set3DNumListeners: [TYPE_VOIDP],
    # System_Set3DRolloffCallback: [TYPE_VOIDP],
  ...
  def self.load_library(library = nil, directory = nil)
    if library.nil?
      ...
      when :WINDOWS then 0.size == 4 ? 'fmod.dll' : 'fmod64.dll'
      when :MACOSX then 'libfmod.dylib'
      when :LINUX then 'libfmod.so'
      else 'fmod'
      end
    end
    library = File.join(directory, library) unless directory.nil?
  ...
  ##
  # Retrieves the calling convention used for making native library calls, which
  # may vary on different platforms.
  # @return [Integer]
  # @see Fiddle::DEFAULT
  # @see Fiddle::STDCALL
  def self.calling_convention
    @abi
  end
end
